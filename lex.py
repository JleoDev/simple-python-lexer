...
class Token:
    ...
    def __str__(self) -> str:
        return f"Tipo: {self.type} {' '*(13-len(self.type))} -- Valor: {self.value}\n"
    ...
    def _search_tokens(self) -> None:
        ...
        while self._position < len(self._code):
            ...
            if caracter.isdigit():
                ...
                self.list_token_types.append(Token(type=Token_types.TOK_NUM, value=int(value)))
            elif caracter in "+-*/":
                self.list_token_types.append(Token(type=Token_types.TOK_OP, value=self._tokens_constants.get(caracter)))
                caracter = self._next_caracter()
            elif caracter in "()":
                self.list_token_types.append(Token(type=Token_types.TOK_PONT, value=self._tokens_constants.get(caracter)))
                caracter = self._next_caracter()
            else:
                self.list_token_types.append(Token(type=Token_types.TOK_INDEF, value=caracter))
                caracter = self._next_caracter()
